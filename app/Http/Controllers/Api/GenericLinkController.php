...
class GenericLinkController extends Controller
{
    ...
    /**
     * @OA\Get(
     *     path="/api/generic/links",
     *     summary="Récupérer les liens par défaut",
     *     description="Retourne la liste paginée des liens génériques",
     *     tags={"Links"},
     *     @OA\Response(
     *         response=200,
     *         description="Liste des liens récupérés avec succès",
    ...
    {
        $links = GenericLink::with(['category', 'country'])
            ->where('status', 1)
            #->where('validation_status', 'approved')
            ->paginate(10);
        return response()->json([
